...
class HashTag extends Component {
  static async getInitialProps({ req, res, query: { tag } }) {
    try {
      const topics = await axios.get(config.getTopicByTag + "?tag=" + tag);
  ...
  handleFetchMore = async () => {
    const { topics, setCurrentPage, setTopics, tag } = this.props;
    ...
    try {
      const nextTopics = await axios.get(config.getTopicByTag + "?tag=" + tag);
