    ...
    <Card className={classes.card} elevation={0}>
        ...
        <div className={classes.contentBody}>
          ...
          {topic.siteId && (
            <a className={classes.link} href={"/domain?s=" + topic.siteId.url}>
              {" "}
              ({topic.siteId ? topic.siteId.url : topic.url}){" "}
            </a>
          )}
          {!topic.siteId && (
            <a className={classes.link} href={"/domain?s=" + topic.url}>
              {" "}
              {topic.url}
            </a>
          )}
      ...
      <Hidden smDown>
        <CardMedia
          ...
          href={"/topics/" + topic._id + "/" + nutralizeTitle(topic.title)}
          target="_blank"
          image={topic.imgThumbnail ? (config.getImage + topic.imgThumbnail) : ("https://image.thum.io/get/iphoneX/width/200/auth/3228-www.tipestry.com/" + topic.url)}
        />
      </Hidden>
    </Card>
  );
