...
class DisplayCommentTips extends Component {
  constructor(props) {
    super(props);
  ...
  handleFetchTips = async () => {
    ...
    try {
      const tip = await axios.get(config.tipComment + "/" + commentId);
...
export default connect(mapStateToProps)(withStyles(styles)(DisplayCommentTips));
