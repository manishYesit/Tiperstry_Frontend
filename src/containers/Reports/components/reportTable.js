...
export default function StickyHeadTable({ reportedUsers, user, deleteReport, setCurrentId, currentid }) {
                  ...
                  return (row ?
                    <TableRow hover role="checkbox" tabIndex={-1} key={row.code}>
                      <TableCell key={columns[0].id} align={columns[0].align}>
                        <Link href={`/topics/${row.topicId}/${row.topicTitle}`}>
                          {row.topicTitle}
                        </Link>
                      </TableCell>
                      <TableCell key={columns[1].id} align={columns[1].align}>
                        {row.reportedUserId &&
                          <Link href={`/p/${row.reportedUserId.username}`}>
                            {row.reportedUserId.username}
                          </Link>
                        }
                      </TableCell>
                      <TableCell key={columns[2].id} align={columns[2].align}>
                        {row.commentId ? row.commentId.ip : row.topicId.ip}
                      </TableCell>
      ...
      >
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            {"Are you sure you want to delete the topic assoicated with this report ?"}
          </DialogContentText>
        </DialogContent>
        <DialogActions>
