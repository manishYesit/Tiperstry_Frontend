...
const SignInSide = ({ res, setRes, email, setRoute, handleEmail, setToken, t }) => {
  ...
  const handleChange = val => (event) => {
	...
	const handleLogin = async (event) => {
		...
		try {
      setLoading(true);
      const reset = await axios.post(config.resetAccountOtp, { email });
  ...
  return (
    <form className={classes.form} onSubmit={handleLogin} noValidate>
      ...
      >
        {loading ? <CircularProgress size={25} /> : t("Get OTP")}
      </Button>
      <Grid container>
        <Grid item xs>
